...
function Create(props: { replyingTo?: NostrEvent }) {
          ...
          <Switch>
            ...
            <Match when={!previewUrl()}>
              <Button
                ...
                size="lg"
                class={`${props.replyingTo && !isRecording() ? "h-8 w-8 bg-transparent text-black hover:bg-accent shadow-none rounded-md" : "h-16 w-16 shadow-lg rounded-[50%]"} transition-transform duration-200 flex items-center justify-center p-0 ${isRecording() ? "bg-destructive hover:bg-destructive/90" : ""}`}
              >
                <Switch>
